...
| 5 | ML model as regime/horizon selector | GATED | Do not start generic ML optimization; first discover interpretable CPR-favorable regimes and their sensitivity/specificity, then validate them chronologically |
| 5A | Conditional CPR-favorable regime discovery | IN PROGRESS | Identify interpretable, non-leaky CPR contexts with favorable sensitivity/specificity and positive conditional expectancy using TRAIN-only rule discovery, then evaluate the unchanged rules on validation/test periods |
| 5B | Regime-specific decision-rule validation | PLANNED | Candidate regimes retain sensitivity/specificity and conditional expectancy on chronological OOS data |
| 6 | Robustness of discovered favorable regimes | PLANNED | Surviving regimes remain stable under dependence-aware bootstrap, subperiods, volatility, and parameter perturbation |\n| 7 | Final walk-forward / untouched holdout | PLANNED | No material degradation on unseen data |\n| 8 | Paper-trading signal pipeline | PLANNED | Reproducible live signal generation with explicit execution rules |

## Current step: Phase 9I — Prospective chronological validation
...
\n- 2026-09-18: Phase 6 robustness implementation added. Pre-specified diagnostics include 10-session block bootstrap, fixed 2015–2019 versus 2020–2024 chronological stability, volatility terciles using pre-existing D_ATR20/close, LONG/SHORT decomposition, and symmetric ±20% CPR threshold perturbations around the frozen 0.50/1.00 thresholds. No configuration is selected as a winner.\n\n- 2026-09-18: Research objective refined from testing whether CPR works uniformly across regimes to discovering which objectively defined market contexts make the frozen CPR signal most useful for trade decisions. Phase 5A added an interpretable shallow decision-tree regime discovery process over fixed intraday and swing horizons, chronological TRAIN/VALIDATION/TEST splits, sensitivity/specificity Pareto frontiers, and conditional R outcomes. The purpose is regime identification, not global CPR acceptance/rejection and not generic parameter optimization.\n\n\n## Phase 9I gate\n\nThe next controlled gate is a fresh chronological holdout for the frozen Phase 9H candidate LEAF_4 / SHORT / 10-session / ATR 1R stop + 2R target. The hard cutoff is 2026-09-17 15:30 Asia/Kolkata. The phase uses the latest immutable upstream 2026 source commit on each run, but candidate rules are never refit from that data. A readiness-only run with zero completed trades is not interpreted as a strategy result; the workflow continues automatically until the holdout matures.\n