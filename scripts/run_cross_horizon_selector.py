...
def main()->None:
    ...
    oos_days=set(oos.entry_day); rows=[]; daily=oos.groupby("signal_day",as_index=False).return_R.mean(); rows.append({"strategy":"SELECTED","signals":len(oos),"signal_days":len(daily),"mean_R":daily.return_R.mean(),"median_R":daily.return_R.median(),"win_day_rate":(daily.return_R>0).mean()})
    for h in HORIZONS:
        q=e[e.entry_day.isin(oos_days)&(e.horizon==h)]; d=q.groupby("signal_day",as_index=False).return_R.mean(); rows.append({"strategy":h,"signals":len(q),"signal_days":len(d),"mean_R":d.return_R.mean(),"median_R":d.return_R.median(),"win_day_rate":(d.return_R>0).mean()})
