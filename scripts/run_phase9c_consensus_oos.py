...
def main():
    ...
    ap.add_argument("--p7",required=True); ap.add_argument("--p8",required=True); ap.add_argument("--output-dir",required=True)
    ...
    for label,path in [("PHASE7_INDEPENDENT",a.p7),("PHASE8_UNTOUCHED_2026",a.p8)]:
        ...
        if req-set(d.columns): raise ValueError(f"{label} missing {sorted(req-set(d.columns))}")
        d["dataset"]=label
        d["signal_time"]=pd.to_datetime(d.signal_time); d["signal_day"]=pd.to_datetime(d.signal_day)
        frames.append(d)
    ...
    def st(a,b):
        if a==0 and b==0:return "NO_SIGNAL"
        if a and b:return "CONFLICT"
        n=max(a,b); side="LONG" if a else "SHORT"
    ...
    d=d.merge(g[["dataset","signal_time","signal_day","state"]],on=["dataset","signal_time","signal_day"],how="left")
    rows=[]
    for (dataset,state,asset,horizon,side),q in d.groupby(["dataset","state","asset","horizon","side"]):
        x=q.realized_return_R.dropna().to_numpy()
        lo,hi=bootstrap_ci(x,seed=20260918+hash(dataset+state+asset+horizon+side)%1000)
        rows.append(dict(dataset=dataset,state=state,asset=asset,horizon=horizon,side=side,
                         observations=len(x),mean_R=float(np.mean(x)) if len(x) else np.nan,
                         median_R=float(np.median(x)) if len(x) else np.nan,
                         positive_fraction=float(np.mean(x>0)) if len(x) else np.nan,
                         mean_R_ci95_low=lo,mean_R_ci95_high=hi))
    ...
    wide=result.pivot_table(index=["state","asset","horizon","side"],columns="dataset",values="mean_R")
    ...
    pd.DataFrame([{"taxonomy":"Fixed Phase 9B taxonomy; no refitting or threshold adjustment.",
                    "datasets":"Phase 7 independent replication + Phase 8 untouched 2026 holdout",
                    "candidate_selection":"NONE","outcome_based_selection":"NONE",
                    "cross_horizon_pooling":"NONE"}]).to_csv(out/"phase9c_provenance.csv",index=False)
    print("=== PHASE 9C ===")
