...
def add_consensus(events, frontier, dataset_label):
    rows = []
    frontier_records = frontier.to_dict("records")
    for asset, ev in events.items():
        if ev.empty:
            continue
        for ts, g in ev.groupby("signal_time"):
            ...
            c = g.iloc[0]
            activated = [
                cand for cand in frontier_records
...
def main():
    ...
    for name, path in [("PHASE7_INDEPENDENT", a.p7), ("PHASE8_UNTOUCHED_2026", a.p8)]:
        ...
        pop = add_consensus(ev, frontier, name)
    ...
    actual = set(d["dataset"].dropna().unique())
    if actual != expected:
        raise ValueError(f"Dataset labeling failure: expected {sorted(expected)}, got {sorted(actual)}")
